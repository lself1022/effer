...
import { Chunk, Data, Effect, Match, Stream } from "effect";
...
describe('State module', () => {
  describe('reducer function', () => {
    it.scoped('should update state based on dispatched messages', () => Effect.gen(function* () {
      ...
      const counter = yield* State.reducer(initialState, (state, msg) => Match.type().pipe(Match.tag('INCREMENT', () => Effect.succeed({
        ...state,
        count: state.count + 1
      })), Match.exhaustive)(msg));
      const getCount = (yield* Stream.toPull(counter.stream)).pipe(Effect.andThen(chunk => Chunk.head(chunk)), Effect.andThen(state => state.count));
  ...
  describe('simple function', () => {
    it.scoped('should set new value with set method', () => Effect.gen(function* () {
      const counter = yield* State.simple(0);
      const getCount = (yield* Stream.toPull(counter.stream)).pipe(Effect.andThen(chunk => Chunk.head(chunk)));
      let result = yield* getCount;
      expect(result).toEqual(0);
      counter.set(1);
      result = yield* getCount;
  ...
  describe('async function', () => {
    it.scoped('should return loading and success states for async effect', () => Effect.gen(function* () {
      ...
      const asyncEffect = Effect.promise(asyncFn);
      const result = State.async(asyncEffect);
      const getState = (yield* Stream.toPull(result.stream)).pipe(Effect.andThen(chunk => Chunk.head(chunk)));
    ...
    it.scoped('should return error state for failed async effect', () => Effect.gen(function* () {
      class TestError extends Data.Error {}
